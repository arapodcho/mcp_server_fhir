...
@mcp.tool()
async def get_family_member_history(patientId=None, input_id=None):
    """
    ...
    Args:
        patientId: The FHIR Logical ID of the patient. Use this to get all medication records for a specific person.
        input_id: The specific FHIR Resource ID for a single FamilyMemberHistory. If provided, the search focuses on this specific item.        
    """
    await ensure_auth()
    if patientId is None and input_id is None:
        return "Error: You must provide either patientId or input_id"
